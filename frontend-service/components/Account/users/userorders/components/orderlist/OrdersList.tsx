...
const OrdersList = ({ setorder_details_id, Orders }: props) => {
  const { Count, Data, HasNext } = Orders;
  const cookies = new Cookies();
  function LoadMore() {}
  return (
    <div className="w-full bg-white h-full drop-shadow-md p-5 space-y-5 ">
      <h1 className="text-zinc-700 font-semibold   text-lg  ">User Orders</h1>
      <div className="relative overflow-x-hidden space-y-4 overflow-y-auto h-[calc(100%-60px)] customscrollbar p-4 ">
        ...
        {Data.length === 0 && (
          <div className=" h-full items-center flex flex-col gap-2 -translate-y-[50px] justify-center">
            <svg className="w-[100px] h-[100px]" xmlns="http://www.w3.org/2000/svg" data-name="Layer 1" width="647.63626" height="632.17383" viewBox="0 0 647.63626 632.17383">
            ...
            <div className="text-center text-gray-500">Oops No Orders Found</div>
          </div>
        )}
        <div className="flex absolute bottom-2 left-1/2  -translate-x-1/2  w-full mt-5 justify-center items-center ">
          {HasNext ? (
