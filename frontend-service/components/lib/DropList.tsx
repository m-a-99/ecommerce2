    ...
    <div className="relative" ref={DropRef}>
      <div onClick={onclick}>
        <div className="px-4 py-2 h-10 ">
          <div className=" text-gray-500 overflow-hidden whitespace-nowrap text-ellipsis ">{inputval ? "" : ValueState.Name}</div>
        </div>
        <input
          value={inputval}
          onKeyDown={(e) => {
          ...
          <div className={` z-10 absolute top-[7px] overscroll-contain rounded-md border-gray-300 border w-full bg-white drop-shadow-md overflow-auto max-h-[250px] py-2`}>
            {ListState.map((v, index) => (
              <div onClick={() => choose(v)} key={v.Id} className=" hover:bg-gray-100 cursor-pointer select-none">
                {Value === v.Id ? <div className={`${hover === index ? "bg-gray-100" : "bg-gray-200/90"} p-3  text-gray-500 border-b `}>{v.Name}</div> : <div className={` ${hover === index ? "bg-gray-100" : ""} p-3 text-gray-500 border-b `}>{v.Name}</div>}
              </div>
            ))}
            {ListState.length == 0 && (
