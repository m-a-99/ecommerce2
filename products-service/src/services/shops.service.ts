...
export class shopsService {
  ...
  private async RPCcheckIsShopsExist(Ids: string[]) {
    const resData = await this.RPCcall("CheckIsShopsExist", JSON.stringify({ Ids }));
    return resData;
  }
  ...
  async ShopsValidation(Ids: string[]) {
    if (Ids && Ids.length > 0) {
      for (let i = 0; i < Ids.length; i++) {
        if (!Ids[i]) {
      ...
      const checkExist = await this.RPCcheckIsShopsExist(Ids);
      for (let v of checkExist) {
        if (!v.Exist) {
          throw new BadRequestError(v.message);
        }
