...
export class productsService {
  ...
  private async  RPCgetShopProductsCountByIds(Ids:string[]){
    const resData = await this.RPCcall("GetShopProductsCountByIds", JSON.stringify({ Ids }));
    return resData
  };
  ...
  async getShopProductsCountByIds(Ids: string[]) {
    const count = await this.RPCgetShopProductsCountByIds(Ids);
    return count
  }
}
